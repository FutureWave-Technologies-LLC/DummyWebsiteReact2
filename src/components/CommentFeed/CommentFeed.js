...
function CommentFeed(props) {
    ...
    function getCommentFeed() {
        ...
        if (commentId) {
            setIsReplyFeed(true)
            axios.get("https://d1ls0cg8xyipo0.cloudfront.net/posts/replies/", {
        ...
        //get comments for post based on postId
            ...
            axios.get("https://d1ls0cg8xyipo0.cloudfront.net/posts/comments/", {
    ...
    const handleCommentSubmit = async (e) => {
        ...
        if (modalInput.trim() != "") {
            ...
            try {
                if (commentId) {
                    await axios.post('https://d1ls0cg8xyipo0.cloudfront.net/posts/replies/', {
                        user_id: token.user_id,
                        comment_id: commentId,
                        reply: modalInput
                    })
                } else {
                    await axios.post('https://d1ls0cg8xyipo0.cloudfront.net/posts/comments/', {
                        user_id: token.user_id,
                        post_id: postId,
                        comment: modalInput
                    })
