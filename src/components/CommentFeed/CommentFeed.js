...
function CommentFeed(props) {
    ...
    function getCommentFeed() {
        ...
        if (commentId) {
            setIsReplyFeed(true)
            axios.get("http://18.219.191.19:8000/posts/replies/", {
        ...
        //get comments for post based on postId
            ...
            axios.get("http://18.219.191.19:8000/posts/comments/", {
    ...
    const handleCommentSubmit = async (e) => {
        ...
        if (modalInput.trim() != "") {
            ...
            try {
                if (commentId) {
                    await axios.post('http://18.219.191.19:8000/posts/replies/', {
                        user_id: token.user_id,
                        comment_id: commentId,
                        reply: modalInput
                    })
                } else {
                    await axios.post('http://18.219.191.19:8000/posts/comments/', {
                        user_id: token.user_id,
                        post_id: postId,
                        comment: modalInput
                    })
